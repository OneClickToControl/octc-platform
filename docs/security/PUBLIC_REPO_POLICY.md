...
Los nombres `health-app`, `health-app-agents`, `store-app`, `strategy-app`, `openclaw-workspace-enlaza`, `ats-copilot-agent`, `1click2control` son repos privados y **no deben aparecer en este repo público** salvo:

- Anonimizados como `<product>` o `<acp-id>` en patrones genéricos.
- En el archivo de stubs `docs/PORTFOLIO.md` (que apunta al privado) — ese stub es la única excepción y está sellado.
- En historia git ya pasada (no la reescribimos por costo/riesgo; nuevas referencias quedan bloqueadas).
...
4. Re-escribir historia con `git filter-repo --invert-paths --path <archivo>` y force-push (requiere coordinación porque rompe SHAs públicos).
